...
value (Num n) _ = n
value (Var variable) dic =
 case Dictionary.lookup variable dic of
  Nothing -> error ("Undefined variable " ++ variable)
  Just k -> k
value (Add t u) dic =
 (value t dic) + (value u dic)
value (Sub t u) dic =
 (value t dic) - (value u dic)
value (Mul t u) dic =
 (value t dic) * (value u dic)
value (Div t u) dic
 | value u dic == 0 = error ("division by 0")
