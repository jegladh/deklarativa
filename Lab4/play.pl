...
play :-
 initialize(InitState,Plyr), playgame(Plyr,InitState). 
...
playgame(Plyr,State) :- 
  ...
  writeln(Move), 
  nextState(Plyr,Move,State,NewState,NextPlyr,AllDir), 
...
% - if move is invalid, recall getmove until a valid move is input 
 ...
 %etmove(1,State,Move) :- 
 % showState(State), 
  %writeln('NotComputer is moving...'), 
  %mmeval(1,State,_,Move,3,SeF), 
  %write('NotCompute Move computed by searching '), 
  %write(SeF), 
  %writeln(' states.'). 
...
getmove(1,State,Move) :- 
 ...
 read(Proposed), 
  validmove(1,State,Proposed,Direction), !, 
  Move = Proposed. 
...
evalMoves(1,St,[Mv|Rest],ValSoFar,MvSoFar,Val,BestMv,D,Se,SeF) :- 
  nextState(1,Mv,St,NewSt,NextPlyr,AllDir), !,
...
evalMoves(2,St,[Mv|Rest],ValSoFar,MvSoFar,Val,BestMv,D,Se,SeF) :- 
  nextState(2,Mv,St,NewSt,NextPlyr,AllDir), !,
